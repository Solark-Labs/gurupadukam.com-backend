...
app.put('/api/classes/:id', authenticateToken, requireAdminOrSuper, async (req, res) => {
  const classId = req.params.id;
  const { title, instructor_name, time, fee, image, description } = req.body;
...
app.delete('/api/classes/:id', authenticateToken, requireAdminOrSuper, async (req, res) => {
  const classId = req.params.id;
  try {
    const cls = await dbGet("SELECT * FROM classes WHERE id = ?", [classId]);
    if (!cls) {
    ...
    if (req.user.role !== 'super_admin' && cls.proposer_name !== req.user.name) {
