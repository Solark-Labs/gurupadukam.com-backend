...
app.post('/api/auth/register', async (req, res) => {
  const { name, email, password, phone, role, location, specialization, fee, image, cottageCategory, cottageAddress, cottageCapacity, otp, emailCode, firebaseToken } = req.body;
...
app.post('/api/auth/email-login/send', async (req, res) => {
  ...
  const isMock = !emailSent || (!process.env.SENDGRID_API_KEY && !process.env.SMTP_HOST);
  res.json({ 
  ...
  try {
    ...
    try {
      ...
      for (const item of cartItems) {
        itemsHtml += `
          <tr>
            <td style="padding: 10px; border-bottom: 1px solid #eee; font-size: 13px;">${item.name}</td>
            <td style="padding: 10px; border-bottom: 1px solid #eee; font-size: 13px; text-align: center;">${item.quantity}</td>
            <td style="padding: 10px; border-bottom: 1px solid #eee; font-size: 13px; text-align: right;">₹${item.price}</td>
            <td style="padding: 10px; border-bottom: 1px solid #eee; font-size: 13px; text-align: right;">₹${item.price * item.quantity}</td>
          </tr>
        `;
      }
      ...
      const receiptHtml = `
        <div style="font-family: Arial, sans-serif; padding: 25px; border: 2px solid #C9943A; border-radius: 12px; max-width: 600px; background-color: #FCFBF8; margin: auto;">
          ...
          <div style="margin: 20px 0; background: #fff; border: 1px solid #e0e0e0; border-radius: 6px; padding: 15px; font-size: 12px; line-height: 1.6;">
            ...
            <strong>Payment Method:</strong> ${paymentMethod} (${paymentStatus === 'paid' ? 'Paid' : 'Payment Pending'})<br>
            <strong>Logistics Status:</strong> Registered on Shiprocket 🚚<br>
            <strong>AWB Number:</strong> ${shipment.awb_code || 'Pending Dispatch'}
          </div>
      ...
      sendEmailNotification(customerEmail, receiptSubject, receiptHtml);
      sendSMSNotification(customerPhone, `Hari Om! Order ${orderId} confirmed for ₹${total}. Shipped via Shiprocket (AWB: ${shipment.awb_code || 'Pending'}). Track details at gurupadukam.com ✦`);
    } catch (notifErr) {
      console.error('[Notification Dispatch Failed on Checkout]:', notifErr.message);
    }
...
app.post('/api/classes/:id/register', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    try {
      const subject = `✦ Confirmed Enrollment: Gurukulam Batch – ${cls.title} 🌿`;
      const htmlContent = `
        <div style="font-family: Arial, sans-serif; padding: 25px; border: 2px solid #C9943A; border-radius: 12px; max-width: 600px; background-color: #FCFBF8; margin: auto;">
          <h2 style="color: #5C0A20; text-align: center; border-bottom: 2px solid #C9943A; padding-bottom: 12px; margin-top: 0;">gurupadukam.com</h2>
          <p style="font-size: 15px; color: #1A1A1A; font-weight: bold;">Hari Om, ${req.user.name} ji!</p>
          <p style="font-size: 13px; color: #333; line-height: 1.5;">Congratulations! Your seat has been successfully reserved in our sacred Gurukulam study batch for **${cls.title}**.</p>
...
app.post('/api/purohits/:id/book', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const meetLink = mode === 'Online'
      ? "https://meet.google.com/gp-" + Math.random().toString(36).substr(2, 4) + "-" + Math.random().toString(36).substr(2, 3)
      : null;
    ...
    try {
      ...
      const devoteeHtml = `
        <div style="font-family: Arial, sans-serif; padding: 25px; border: 2px solid #C9943A; border-radius: 12px; max-width: 600px; background-color: #FCFBF8; margin: auto;">
          ...
          <div style="margin: 20px 0; background: #fff; border: 1px solid #e0e0e0; border-radius: 6px; padding: 15px; font-size: 12px; line-height: 1.6;">
            ...
            <strong>Ritual Venue Address:</strong> ${address}
          </div>
      ...
      if (devoteePhone) {
        sendSMSNotification(devoteePhone, `Hari Om! Puja booking ${bookingId} confirmed with ${purohit.name} for ${poojaType} on ${bookingDate}. Details sent to email ✦`);
      }
      ...
      if (priestUser && priestUser.email) {
        ...
        if (priestUser.phone) {
          sendSMSNotification(priestUser.phone, `Hari Om Acharya ji! New booking ${bookingId} for ${poojaType} on ${bookingDate} with devotee ${devoteeName}. Contact details in email ✦`);
        }
      }
    } catch (notifErr) {
